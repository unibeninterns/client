...
import { articleViewsApi, facultyApi } from '@/lib/api';
import { Eye, Calendar, User, ArrowLeft, Share2, BookOpen, Clock } from 'lucide-react';
    ...
    const processArticle = async () => {
      if (initialArticle) {
        ...
        if (initialArticle.content) {
          setReadingTime(calculateReadingTime(initialArticle.content));
        }
      ...
      <div className="min-h-screen bg-gradient-to-br from-fuchsia-50 to-white">
        <div className="container mx-auto px-4 py-8 max-w-4xl">
          <Alert variant="destructive" className="mb-6 border-red-200 bg-red-50">
            <AlertDescription className="text-red-700">Article not found</AlertDescription>
          </Alert>
          <Button
            variant="outline"
            onClick={() => router.back()}
        ...
        <div className="mb-8">
          ...
          <div className="flex flex-wrap gap-3 sm:gap-4 mb-6 text-sm text-gray-600">
            <div className="flex items-center bg-white px-3 py-1 rounded-full shadow-sm">
              ...
              <span className="font-medium">{article.owner?.username || 'Anonymous'}</span>
            </div>
              ...
              {article.contributors.map((contributor) => (
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    {contributor.bio && (
                      <p className="text-sm mt-1 text-gray-700 line-clamp-2">{contributor.bio}</p>
                    )}
                  </div>
                </div>
              ))}
